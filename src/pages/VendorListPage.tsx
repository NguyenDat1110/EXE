...
export default function VendorListPage({ navigate }: { navigate: (page: string, params?: any) => void }) {
  const [activeFilter, setActiveFilter] = useState('Tất cả');
  const [searchQuery, setSearchQuery] = useState('');

  const filteredVendors = vendors.filter(v => {
    const matchCategory = activeFilter === 'Tất cả' || v.category === activeFilter || 
      (activeFilter === '< 50M VND' && parseInt(v.budget) < 50) ||
      (activeFilter === '50M - 100M VND' && parseInt(v.budget) >= 50 && parseInt(v.budget) <= 100) ||
      (activeFilter === '> 100M VND' && parseInt(v.budget) > 100);
  ...
  return (
    <div className="min-h-screen pt-24 pb-12 px-6 lg:px-20 max-w-7xl mx-auto w-full">
      <div className="flex flex-col md:flex-row justify-between items-end mb-12 gap-6">
        <div>
          <h2 className="text-4xl font-serif italic text-white mb-4">Danh Sách Đối Tác</h2>
          <p className="text-slate-400 max-w-xl">Khám phá và đặt dịch vụ từ các nhà cung cấp sự kiện hàng đầu.</p>
        </div>
        ...
        <div className="flex flex-col sm:flex-row gap-4 w-full md:w-auto">
          <div className="relative">
            ...
            <input 
              type="text" 
              placeholder="Tìm kiếm vendor, concept..." 
              value={searchQuery}
              onChange={(e) => setSearchQuery(e.target.value)}
              className="w-full sm:w-64 bg-white/5 border border-white/10 rounded-xl py-2 pl-10 pr-4 text-sm text-white focus:outline-none focus:border-primary/50 transition-colors"
            />
          </div>
          <button className="flex items-center justify-center gap-2 px-4 py-2 glass-card rounded-xl text-sm font-medium hover:bg-white/10 transition-colors">
