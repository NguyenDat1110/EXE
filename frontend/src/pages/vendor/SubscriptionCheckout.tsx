...
export function SubscriptionCheckout() {
  ...
  const planCode = (location.state?.plan as string) || '';
  ...
  const currentPlan = plans.find((p) => p.code === planCode || p.type === planCode || p._id === planCode);
  const vendorActivePlan = plans.find((p) => p.code === vendor?.subscriptionPlan) || plans.find((p) => p.type === vendor?.subscriptionPlan) || null;
  ...
  const handlePayOSCheckout = async () => {
    ...
    try {
      if (vendor && vendor.subscriptionStatus === 'active' && vendor.subscriptionExpiry && new Date(vendor.subscriptionExpiry) > new Date() && vendorActivePlan?._id === currentPlan?._id) {
      ...
      if (currentPlan?.price === 0) {
        const response = await api.post('/subscription/update', { plan: planCode });
        if (response.data) {
          setSuccess(true);
      ...
      const response = await createSubscriptionPayment(planCode);
    ...
    return (
      <div className="min-h-screen bg-navy py-12 px-4 flex items-center justify-center">
        <motion.div initial={{ scale: 0.9, opacity: 0 }} animate={{ scale: 1, opacity: 1 }}
          ...
          <p className="text-silver/60 mb-6">Bạn đã nâng cấp lên gói {currentPlan?.name || planCode}.</p>
      ...
      <div className="max-w-4xl mx-auto">
        <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="space-y-8">
          <div>
            ...
            <p className="text-silver/60">Hoàn tất thanh toán để kích hoạt gói {currentPlan?.name || planCode}</p>
          </div>
            ...
            <motion.div initial={{ opacity: 0, x: -20 }} animate={{ opacity: 1, x: 0 }} className="lg:col-span-1 space-y-6">
              <div className="glass-panel p-6 rounded-2xl">
                ...
                <div className="space-y-3 mb-4 pb-4 border-b border-white/10">
                  ...
                  {currentPlan?.price === 0 ? (
                    ...
                    <>
                      <div className="flex justify-between text-silver/70">
                        <span>Giá gói:</span>
                        <span className="text-white font-medium">{currentPlan?.price.toLocaleString('vi-VN')}₫</span>
                      </div>
                      <div className="flex justify-between text-silver/70">
                        <span>Thuế (0%):</span>
                        <span>0₫</span>
                ...
                <div className="flex justify-between items-center">
                  ...
                  <span className={`text-2xl font-bold ${currentPlan?.price === 0 ? 'text-emerald-400' : 'text-cyan'}`}>
                ...
                <div className="mt-4 p-3 bg-cyan/10 rounded-lg border border-cyan/30">
                  <p className="text-cyan text-sm">✓ Thời hạn: {currentPlan?.duration} ngày</p>
                </div>
              </div>
            </motion.div>
              ...
              <div className="glass-panel p-8 rounded-2xl space-y-6">
                ...
                <div className="flex items-start gap-4 p-4 rounded-xl border-2 border-cyan/40 bg-cyan/5">
                  ...
                  <div className="flex-grow">
                    ...
                    <p className="text-silver/60 text-sm mt-1">Quét mã QR hoặc chuyển khoản qua cổng thanh toán PayOS.</p>
                  </div>
                </div>
                ...
                <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }}
                  ...
                  <div className="flex items-start gap-3">
                    ...
                    <div className="text-sm text-silver/70 space-y-2">
                      ...
                      <p>Hỗ trợ quét QR bằng mọi ứng dụng ngân hàng (VietQR).</p>
                    </div>
                  </div>
                </motion.div>
                ...
                {error && (
                  <div className="bg-red-500/10 border border-red-500/30 rounded-lg p-4">
                    <p className="text-red-400 flex items-center gap-2"><AlertCircle className="w-5 h-5" />{error}</p>
                  </div>
                )}

                <button onClick={handlePayOSCheckout} disabled={isProcessing}
                  ...
                  {isProcessing ? (
                    ...
                    <><Check className="w-5 h-5" />Kích Hoạt Gói Miễn Phí</>
                  ) : (
